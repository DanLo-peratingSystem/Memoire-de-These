...
\pagebreak            
    ...
    \subsection{Spécification des paramètres de Contrôle}
    	...
    	\paragraph{\ctxt{1} -- Système exécuté en mode dégradé}
    		...
    		\end{equation}
    	%% Par ailleurs, si d'aventure le Task Wrapper Component devrait gérer en plus des timestamps de fin, les timestamps de début d'exécution des tâches critiques, alors la condition deviendrait la suivante~: 
    	%%  \begin{equation}\label{eq:conditionsMemoireControleBIS}
	    	%		L_{buf} = \sum_{i=1}^{n}\left\lceil \frac{T_{CCC}+WCRT(\tau_i)}{T_i} \right\rceil + $\sum_{i=1}^{n} \left\lceil \frac{T_{CCC}}{T_i} \right\rceil$
    	%%	\end{equation}

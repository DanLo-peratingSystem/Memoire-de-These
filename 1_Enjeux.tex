...
\section{Évolutions des Systèmes embarqués}
	\subsection{Nouveaux systèmes intelligents et connectés}
		...
		\paragraph{Multicœurs "classiques"}
			...
			A l'inverse, les architectures basées sur la cache se veulent polyvalentes et comptent sur des mécanismes de contrôle logiciel pour optimiser l'utilisationd des ressources.
...
%		\end{itemize}
		...
		\begin{table}[ht]
		   ...
		   \mrQ{Probabilité} & \red{ E4 }  &    QM   &  \gr{ A }  & \oran{ B } &            & \gr{ A } & \oran{ B } & \oran{ C }  & 			  & \yel{ B } & \oran{ C } &  \red{ D }  \\
				\bottomrule
			\end{tabular}
		\end{table}
